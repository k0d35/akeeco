...
export class TripCreateWizardShellPageComponent implements OnInit, OnDestroy {
  ...
  private _unlocking = signal<boolean>(false);
  unlocking = computed(() => this._unlocking());
  ...
  adminUnlockLock() {
    if (!this.isAdmin() || this._unlocking()) return;
    this._unlocking.set(true);
    this._autosaveStatus.set('Admin unlock in progress...');
    this.lockMgr.forceReleaseAndAcquire((ok, message) => {
      this._unlocking.set(false);
      if (ok) {
        this._autosaveStatus.set('Draft unlocked. You can now edit.');
        return;
      }
      this._autosaveStatus.set(message || 'Admin unlock failed.');
    });
  }
  ...
  discardDraft() {
    ...
    this.drafts.delete(id).subscribe({
      next: () => { this._serverDraft = null; this._showRestoreBanner.set(false); this.state.reset(); this.router.navigateByUrl('/staff/trips/drafts'); },
  ...
  saveAndExit() {
    const id = this.state.draftId;
    if (!id) { this.router.navigateByUrl('/staff/trips/drafts'); return; }
    if (this.readonly()) { this.router.navigateByUrl('/staff/trips/drafts'); return; }
    ...
    this.drafts.save(id, this.state.draftVersion, this.state.raw()).subscribe({
      next: (saved) => {
        ...
        this.router.navigateByUrl('/staff/trips/drafts');
      },
      error: (err: any) => {
        const msg = String(err?.message || err);
        if (msg.toLowerCase().includes('conflict') || msg.toLowerCase().includes('version')) {
  ...
  submit() {
    ...
    if (draftId) {
      this.drafts.submit(draftId).subscribe({
        next: () => this.drafts.delete(draftId).subscribe({ next: () => this.router.navigateByUrl('/staff/trips'), error: () => this.router.navigateByUrl('/staff/trips') }),
        error: () => this.router.navigateByUrl('/staff/trips'),
      });
    } else {
      this.router.navigateByUrl('/staff/trips');
    }
  }
}
