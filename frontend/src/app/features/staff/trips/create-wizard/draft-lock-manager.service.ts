...
export class DraftLockManagerService implements OnDestroy {
  ...
  start(draftId: string) {
    ...
    const acquireSub = this.api.acquire(draftId, me.id, me.name).subscribe({
      next: (lock) => {
  ...
  forceReleaseAndAcquire(cb?: (ok: boolean, message?: string) => void) {
    if (!this.draftId) { cb?.(false, 'No draft selected.'); return; }
    const id = this.draftId;
    this.api.forceRelease(id).subscribe({
      next: (ok) => {
        if (!ok) { cb?.(false, 'Unable to unlock draft.'); return; }
        this.start(id);
        cb?.(true);
      },
      error: (err: any) => cb?.(false, String(err?.message || err)),
    });
  }
