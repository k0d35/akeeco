...
mutation Acquire($draftId: ID!, $userId: String!, $userName: String!) {
  acquireDraftLock(draftId: $draftId, userId: $userId, userName: $userName) { token userId userName acquiredAt ttlSeconds }
}
`;
...
export const GQL_LOCK_FORCE_RELEASE = `
mutation ForceRelease($draftId: ID!) {
  forceReleaseDraftLock(draftId: $draftId)
}
`;
