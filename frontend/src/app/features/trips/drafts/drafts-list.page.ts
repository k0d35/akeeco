...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <footer class="foot">
        <a routerLink="/app/trips">← Back to Trips</a>
      </footer>
    </div>
  `,
  styles: [`
...
export class DraftsListPageComponent {
  ...
  createNew() {
    this.api.create(false).subscribe({
      next: (d) => this.router.navigate(['/app/trips/new/customer'], { queryParams: { draftId: d.id } }),
      error: (e: any) => this.error = String(e?.message || e)
    });
  }

  resume(d: TripDraftDto) {
    this.router.navigate(['/app/trips/new/customer'], { queryParams: { draftId: d.id } });
  }
