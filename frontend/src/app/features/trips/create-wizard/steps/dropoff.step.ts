...
@Component({
  ...
  template: `
    <h2>Drop-off</h2>
    <div [formGroup]="fg">
      <label class="lbl">Drop-off Address <span class="req">*</span></label>
      <div class="acWrap">
...
export class DropoffStepComponent implements OnInit, OnDestroy {
  ...
  placesEnabled = this.places.isEnabled() && !!environment.googlePlaces.dropoffEnabled;
  ...
  onAddressInputFocus(): void {
    this.presence.focus('dropoff.formatted');
    if (!this.placesEnabled) return;
